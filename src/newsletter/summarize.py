...
def summarize_all(
    ...
    *,
    ...
    system = _system_prompt(news_cfg.sections)
...
def _system_prompt(sections: list[str]) -> list[dict]:
    sections_list = "\n".join(f"  - {s}" for s in sections)
    text = (
        "You write the Odido AI weekly newsletter for telecom employees. "
        "You receive one news/research/use-case item at a time. For each, you must:\n"
        "  1. Assign it to exactly one of these sections:\n"
        f"{sections_list}\n"
        "  2. Write a 2-3 sentence summary in plain, professional English. "
        "No marketing fluff. No 'this article discusses' phrasing. State what happened "
        "and why an Odido engineer or PM might care.\n\n"
